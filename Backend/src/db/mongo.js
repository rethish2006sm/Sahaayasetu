...
async function ensureIndexes(database) {
  await Promise.all([
    ...
    database.collection('users').createIndex({ owner_ngo_user_id: 1 }),
    database.collection('ngos').createIndex({ id: 1 }, { unique: true }),
    database.collection('ngos').createIndex({ owner_user_id: 1 }),
    ...
    database.collection('tasks').createIndex({ id: 1 }, { unique: true, sparse: true }),
    database.collection('tasks').createIndex({ owner_ngo_user_id: 1, created_at: -1 }),
    database.collection('tasks').createIndex({ assigned_worker_id: 1, created_at: -1 }),
    database.collection('workers').createIndex({ id: 1 }, { unique: true }),
    database.collection('workers').createIndex({ linked_user_id: 1 }, { unique: true, sparse: true }),
    database.collection('workers').createIndex({ owner_ngo_user_id: 1, created_at: -1 }),
